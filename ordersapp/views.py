...
def place_order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            timestamp = datetime.now().strftime('%Y%m%d%H%M%S')                            # to convert datetime function into string use = strftime('%Y%M%D%H%M%S')
            ...
            redirect_url = reverse('ordersapp:payment')+f'?data={serialized_data}'
            return redirect(redirect_url)
        
    return render(request,'ordersapp/order_billing.html', {'form' : form})
...
def create_order_pdf(order,cart_items):
    ...
    elements = []
    customer_info = [
        ['Customer Name:',Orders.user.get_full_name()],
        ['Address:', Orders.address],
        ['Phone:', Orders.phone]
      #  ['Orders_number', Orders.order_number]
    ]

    customer_table = Table(customer_info)
    customer_table.setStyle(TableStyle([
        ('ALIGN',(0,0),(-1,-1),'LEFT'),
        ('FONTNAME',(0,0),(-1,0),('Helvtica-Bold')),
        ('BOTTOMSPADDING',(0,0),(-1,0),12)
    ]))
...
def send_order_email(order, cart_items):
    ...
    email_body = "Thankyou for placing the order, Please find the attached inivoce for your references"
...
def payment(request):
    ...
    if request.method == 'POST':
        try:
            ...
            for item in cart_items:
                ...
                orderpet.save()
            cart_items.delete()      # removing the data
        ...
        # print('95',serialized_data)
        if serialized_data:
            data = json.loads(serialized_data)      # converting the JSON structure inot string 
    ...
    return render(request,'ordersapp/payment_page.html', context)



    # WHOLE DATA CONVERT FROM PYTHON TO JSON  USE DUMPS  (JSON.DUMPS)
    # DATA ALREADY IN JSON FORMAT AND CONVERT INTO PYTHON USE DUMP 
